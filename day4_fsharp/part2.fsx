...
let rec playBingo (numbers: int list) (bingos: Bingo list) =
  ...
  | x::xs -> 
    ...
    let filtered_bingos = next_bingos |> List.filter(fun b -> not (evalBingo b))
    if filtered_bingos.Length = 0  then Ok (next_bingos.[0], x) else playBingo xs filtered_bingos
